...
add_action('plugins_loaded', 'wc_payment_gateway_for_visa_init', 0);
function wc_payment_gateway_for_visa_init(){
if ( !class_exists( 'WC_Payment_Gateway' ) ) {return;}
     ...
     */
    if(!class_exists("Alnazer_WC_Gateway_VISA")){
        class Alnazer_WC_Gateway_VISA extends WC_Payment_Gateway {
            private $back_url;
            private $paymentid;
            private $amount;
            private $rand;
            ...
            public function init_form_fields(){
                $this->form_fields = array(
                    ...
                    'gatewayarea'=> array(
                        'title' => __('Area:', 'payment-gateway-for-visa'),
                        'type'=> 'select',
                        //'description' => __('This controls the title which the user sees during checkout.', 'payment-gateway-for-visa'),
                        'default' => 'asia',
                        'options'=>[
                            "americas"=> __('Americas', 'payment-gateway-for-visa'),
                            "europe"=> __('Europe', 'payment-gateway-for-visa'),
                            "asia"=> __('Asia', 'payment-gateway-for-visa'),
                            "other"=> __('Other', 'payment-gateway-for-visa'),
                        ]
                    ),
                    'interaction'=>
                        array(
                            'title' => __("Interaction:", 'payment-gateway-for-visa'),
                            'type'=> 'select',
                            'description' => __('Indicates the operation that you wish to perform during the Hosted Checkout interaction', 'payment-gateway-for-visa'),
                            'default' => 'NONE',
                            'options'=>[
                                "AUTHORIZE"=> __('AUTHORIZE', 'payment-gateway-for-visa'),
                                "NONE"=> __('NONE', 'payment-gateway-for-visa'),
                                "PURCHASE"=> __('PURCHASE', 'payment-gateway-for-visa'),
                                "VERIFY"=> __('VERIFY', 'payment-gateway-for-visa'),
                            ]
                        ),
                    'language'=> array(
                        'title' => __('Language:', 'payment-gateway-for-visa'),
                        'type'=> 'select',
                        'default' => strtolower(substr($_SERVER['HTTP_ACCEPT_LANGUAGE'], 0, 2)),
                        'options'=>$this->getLangList()
                    ),
                    'title' => array(
                        'title' => __('Title:', 'payment-gateway-for-visa'),
                        'type'=> 'text',
                        'description' => __('This controls the title which the user sees during checkout.', 'payment-gateway-for-visa'),
                        'default' => __('VISA', 'payment-gateway-for-visa')
                    ),
                    'description' => array(
                        'title' => __('Description:', 'payment-gateway-for-visa'),
                        'type' => 'textarea',
                        'description' => __('This controls the description which the user sees during checkout.', 'payment-gateway-for-visa'),
                        'default' => __('Pay securely by Credit card through VISA.', 'payment-gateway-for-visa')
                    ),
                    'merchantId' => array(
                        'title' => __('merchantId', 'payment-gateway-for-visa'),
                        'type' => 'text',
                        'description' => __('Given by VISA for your Merchant Account.'),
                        //'default'=>'pay'
                    ),
                    'logo' => array(
                        'title' => __('logo', 'payment-gateway-for-visa'),
                        'type' => 'text',
                        'description' => __('Company Logo.'),
                        'default'=>plugins_url( 'images/logo.png' , __FILE__ )
                    ),
                    'apiusername' => array(
                        'title' => __('apiusername', 'payment-gateway-for-visa'),
                        'type' => 'text',
                        'description' => __('Given by VISA for your Merchant Account.'),
                    ),
                    'password' => array(
                        'title' => __('password', 'payment-gateway-for-visa'),
                        'type' => 'text',
                        'description' => __('Given by VISA for your Merchant Account.'),
                    ),
                );
            ...
            protected function setup_properties() {
                ...
                $this->method_description = __( 'Easy and fast and secure', 'payment-gateway-for-visa' );
            ...
            public  function  visa_conditional_payment_gateways($available_gateways){
                ...
                if($this->is_test == "yes"){
                    $available_gateways[$this->id]->title= $available_gateways[$this->id]->title. " <b style=\\" .__("Test Mode","wc_visa")."</b>";
            ...
            function process_payment($order_id){
                ...
                if(isset($parsed_array['result']) && $parsed_array['result'] == "SUCCESS"){
                    ...
                    $msg['message'] = __('Payment error:','payment-gateway-for-visa').__((isset($parsed_array['error.explanation'])) ? $parsed_array['error.explanation'] : "Payment error try again",'payment-gateway-for-visa');
                    $msg['class'] = 'error';
            ...
            public function form()
            {
                ...
                $msg['message'] = __('Error happened','payment-gateway-for-visa');
                $msg['class'] = 'error';
                if(!isset($_SESSION))
                {
                    session_start();
                ...
                if($order_id && $order){
                    ...
                    foreach ( $order->get_items() as $item ) {
                        $product_name[]= sprintf("%s ".__('Quantity','payment-gateway-for-visa').": %s ".__('Total','payment-gateway-for-visa').": %s",$item['name'] ,$item['quantity'] ,$item['total']);
                    }
                    ...
                    $msg['message'] = __('order not found','payment-gateway-for-visa');
                    $msg['class'] = 'error';
                    $this->set_order_notif($msg);
            ...
            public function admin_options(){
                echo wp_kses(sprintf("<h3>%s</h3>",$this->title),["h3"=>[]]);
                echo wp_kses('<table class="form-table">',["table"=>array("class"=>[])]);
     ...
     **/
    if(!function_exists("alnazer_wc_payment_gateway_for_visa_add_gateway")){
        function alnazer_wc_payment_gateway_for_visa_add_gateway($methods) {
            $methods[] = 'Alnazer_WC_Gateway_VISA';
            return $methods;
        }
    }

    add_filter('woocommerce_payment_gateways', 'alnazer_wc_payment_gateway_for_visa_add_gateway');
}
